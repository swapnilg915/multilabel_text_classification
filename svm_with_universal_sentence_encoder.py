...
class MultilabelTopicClassifier(object):
    ...
    def __init__(self):
        ...
        self.use_obj = spacy_universal_sentence_encoder.load_model('xx_use_md')
    ...
    def save_models(self):
        ...
        models_path = os.path.join(model_dir, "svm_multilabel_use.pickle")
    ...
    def sentence_encoder_features(self, data):
        embeddings_vector = []
        for sent in data:
            embeddings_vector.append(self.use_obj(sent).vector)
        return np.array(embeddings_vector)
    ...
    def trainModel(self):
        ...
        en_time = time.time()
        train_vector = self.sentence_encoder_features(train_sentences)
        test_vector = self.sentence_encoder_features(test_sentences)
